...
clust4 <- read.csv("vol_cluster4.csv")

dir_day3 <- clust4[,-c(1,16,17,19,20)]
...
file.create("Cluster4 Confusion Matrix mlr.csv")

write.csv(combined_cmat_mlr, " Cluster4 Confusion Matrix mlr.csv")
...
file.create("Cluster4 Confusion Matrix dt3.csv")

write.csv(combined_cmat_dt3, "Cluster 4 Confusion Matrix dt3.csv")
...
# Least Error Model : mtry - 5, min.node.size = 10, replace= True, sample.fraction = 0.50, rmse = 0.5458
...
# Best model : mtry - 3, min.node.size = 3, replace = F, sample.fraction = 0.63, rmse = 0.4864
