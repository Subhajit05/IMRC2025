...
clust1 <- read.csv("dir_cluster1.csv")

dir_dr3 <- clust1[,-c(1,16,17,19,20)]
...
mlr3_row_index <- sample(nrow(dir_dr3), size = 0.8*nrow(dir_dr3), replace = F )

train_data3 <- dir_dr3[mlr3_row_index,]

test_data3 <- dir_dr3[- mlr3_row_index,]
...
file.create("Cluster1 Confusion Matrix mlr3.csv")

write.csv(combined_cmat_mlr, "Confusion Matrix mlr3.csv")
...
svm_row_index3 <- sample(nrow(dir_dr3), size = 0.8*nrow(dir_dr3), replace = F )

train_data3 <- dir_dr3[svm_row_index3,]

test_data3 <- dir_dr3[-svm_row_index3,]
...
file.create("Cluster1 Confusion Matrix svm3.csv")
...
dt_row_index3 <- sample(nrow(dir_dr3), size = 0.8*nrow(dir_dr3), replace = F )

train_data3 <- dir_dr3[dt_row_index3,]

test_data3 <- dir_dr3[-dt_row_index3,]
...
file.create("Cluster1 Confusion Matrix dt3.csv")

write.csv(combined_cmat_dt3, "Confusion Matrix dt3.csv")
...
n_features <- length(setdiff(colnames(dir_dr3), "dr3"))
...
rf_row_index3 <- sample(nrow(dir_dr3), size = 0.8*nrow(dir_dr3), replace = F )

train_data3 <- dir_dr3[rf_row_index3,]

test_data3 <- dir_dr3[-rf_row_index3,]
...
# Least Error Model : mtry - 5, min.node.size = 5, replace=False, sample.fraction = 0.50, rmse = 0.5236
...
file.create("Cluster1 Confusion Matrix RF3.csv")

write.csv(combined_cmat_rf3, "Cluster1 Confusion Matrix RF3.csv")
...
dir_dr3$dr3 <- as.factor(dir_dr3$dr3)


# data split (train - 80%; test - 20%)

nb_row_index <- sample(nrow(dir_dr3), size = 0.8*nrow(dir_dr3), replace = F )

train_data3 <- dir_dr3[nb_row_index,]

predictors <- train_data3[,-18]

predictors <- as.matrix(predictors, ncol = 17)

test_data3 <- dir_dr3[- nb_row_index,]
...
# Best model : mtry - 4, min.node.size = 1, replace = F, sample.fraction = 0.50, rmse = 0.4918
